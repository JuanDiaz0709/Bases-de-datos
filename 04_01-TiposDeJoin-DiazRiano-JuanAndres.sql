...
FROM
    matriculas m
        RIGHT JOIN
    alumnos a ON a.id_alumno = m.id_alumno
...
SELECT 
    a.id_alumno, a.nombre, COUNT(m.id_matricula) AS n
FROM
    matriculas m
        RIGHT JOIN
    alumnos a ON a.id_alumno = m.id_alumno
...
SELECT 
    a.id_alumno, a.nombre, m.id_matricula
FROM
    alumnos a
        LEFT JOIN
    matriculas m ON a.id_alumno = m.id_alumno 
...
SELECT 
    c.id_curso, c.nombre_curso, COUNT(a.id_alumno) AS n_alumnos
FROM
    cursos c
        LEFT JOIN
    matriculas m ON c.id_curso = m.id_curso
...
FROM
    productos pr
        LEFT JOIN
    detalle_pedido AS dp ON dp.id_producto = pr.id_producto
WHERE
...
FROM
    ...
    clientes c ON c.id_cliente = p.id_cliente
        LEFT JOIN
    pagos pa ON pa.id_pedido = p.id_pedido
WHERE
    pa.id_pedido IS NULL
...
FROM
    ...
    clientes AS c ON c.id_cliente = p.id_cliente
        LEFT JOIN
    pagos AS pa ON pa.id_pedido = p.id_pedido
...
FROM
    clientes AS c
        LEFT JOIN
    pedidos AS p ON p.id_cliente = c.id_cliente
        LEFT JOIN
    detalle_pedido AS dp ON dp.id_pedido = p.id_pedido
...
SELECT 
    c.nombre AS cliente, c.fecha_registro AS fecha_registro
FROM
    clientes AS c
        LEFT JOIN
    pedidos AS p ON p.id_cliente = c.id_cliente
WHERE
    c.fecha_registro >= '2025-01-01'
        AND c.fecha_registro < '2026-01-01'
